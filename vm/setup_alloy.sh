...
ALLOY_VERSION="v1.4.3"  # Ensure this matches the latest version in the documentation
...
function install_alloy {
    ...
    sudo mv "$TEMP_DIR/alloy-linux-amd64" /usr/local/bin/alloy  # Changed to /usr/local/bin
    ...
    # Set ownership of the alloy directory to the alloy user
...
{
    \"metrics\": {
        \"url\": \"http://${METRICS_SERVER}:9009/api/v1/push\"
    },
    \"logs\": {
        \"url\": \"http://${METRICS_SERVER}:3100/loki/api/v1/push\"
    }
}
EOF"
...
# Function to uninstall Grafana Alloy
